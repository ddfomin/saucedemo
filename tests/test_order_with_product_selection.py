...
def test_buy_selection_item(driver, authorization_url, password, logger):
    logger.info("Запуск теста с выбором определенного товара")
    ...
    # Добавление товара в корзину (с выбором пользователя)
    main_form = MainPage(driver)
    name_main, price_main = main_form.add_selection_item()
